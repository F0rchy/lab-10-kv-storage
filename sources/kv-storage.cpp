...
void dbEditor::addValue(std::string tableName, std::string key,
                        std::string value) {
  DB* db;
...
void dbEditor::showTable(std::string name, std::string path) const {
  ...
  rocksdb::Iterator* it =
      db->NewIterator(rocksdb::ReadOptions(), handles[position]);
...
void dbEditor::readRequest(size_t id,
                           ...
                           DB* inputDb, std::mutex& _mutex,
                           std::queue<Value>& _values, size_t threadsNum) {
...
void dbEditor::hashDataBaseInit() {
  ...
  if (outputTables->size() <= 1) {
    ...
    status =
        DB::Open(options, this->db_path, *inputTables, &inputHandles, &inputDb);
    if (!status.ok()) {
      if (this->arguments.logLevel == boost::log::trivial::error) {
    ...
    assert(status.ok());
    status = DB::Open(options, this->arguments.output, *outputTables,
                      &outputHandles, &outputDb);
    if (!status.ok()) {
      if (this->arguments.logLevel == boost::log::trivial::error) {
        MESSAGE_LOG(this->arguments.logLevel) << status.ToString() << std::endl;
      }
    ...
    for (size_t i = 0; i != this->arguments.threadCount; ++i) {
      std::thread thread(&dbEditor::readRequest, i, std::ref(inputHandles),
                         inputDb, std::ref(this->mutex), std::ref(this->values),
    ...
    for (size_t i = 0; i != this->arguments.threadCount; ++i) {
      std::thread thread(&dbEditor::writeRequest, std::ref(outputHandles),
                         outputDb, std::ref(this->mutex),
                         std::ref(this->values));
      if (thread.joinable()) {
        thread.join();
      }
    ...
    for (auto handle : inputHandles) inputDb->DestroyColumnFamilyHandle(handle);
